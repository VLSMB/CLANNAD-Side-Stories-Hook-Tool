...
hOrgGame dd -1
    .const
MessageTitle db "超巨星汉化组",0
MessageQuestion db "是否要安装汉化补丁？",0
MessageShotCut db "是否要创建桌面快捷方式？",0
MessageWarn db "请仔细阅读上方内容，并勾选“我已同意”！",0
MessageFindError db "未寻找到游戏本体所在路径！请确保原版游戏正常运行再次尝试。",0AH,0DH
                db "或将汉化补丁运行在游戏本体所在文件夹！",0
MessageNoAccess db "释放文件失败！请试试“以管理员身份运行”。",0
MessageSteam db "汉化补丁安装完成，是否将Steam库的启动器链接到汉化补丁上？",0AH,0DH,"请先结束原版程序的运行。请注意，这样的话原版将无法运行，卸载本汉化补丁后可以恢复原版的运行。",0
MessageDone db "汉化补丁安装完成！请双击“RealliveCHS”运行，或者双击桌面上的快捷方式运行。",0
; 用x64dbg生成的
...
DB "ool",0AH,0DH,0AH,0DH,0AH,0DH,0AH,0DH
DB "V1.5汉化补丁更新内容：",0AH,0DH,0AH,0DH
DB "1.游戏窗口可显示当前剧情标题",0AH,0DH,0AH,0DH
DB "2.修复第9章无法自动翻页问题，修复文字错版",0AH,0DH,0AH,0DH
DB "3.优化补丁图片共存算法，提高补丁运行速率",0AH,0DH,0AH,0DH
DB "4.修改错别字及用法错误的标点",0AH,0DH,0AH,0DH,0AH,0DH,0AH,0DH
DB "V1.6汉化补丁更新内容：",0AH,0DH,0AH,0DH
DB "1.增加了自动寻找游戏本体所在路径的功能，可以在任意目录运行本补丁。（此功能需要游戏本体正在运行）",0AH,0DH,0AH,0DH
DB "2.可以通过Steam库直接运行汉化版。",0
ShotCutText db "[InternetShortcut]",0AH,0DH
            db "URL=%sRealliveCHS.exe",0AH,0DH
            db "IconFile=%sHIKARI_SAKA.ico",0AH,0DH
...
Program03 db "UninstallPatch.bat",0
ProgramOrg db "RealLiveEn.exe",0
ProgramRet db "RealLiveEn.exe.bak",0
ProgramPointer dd offset Program01,offset Program02,offset Program03
...
FindGamePath proc
    ...
    mov handle,eax
    lea esi,lpfile
    invoke GetModuleFileNameExA,handle,NULL,esi,260
    .if eax == 0
        ret
    .endif
    lea esi,lpfile
    ...
    .endw
    mov byte ptr [esi],0
    invoke SetCurrentDirectoryA,addr lpfile
    call FlushCurrentPath
    mov eax,1
    ret
FindGamePath endp

GetGameHandle proc
    ; 由于masm自身的问题，W函数需要手动更改导入表
    local HSnap:DWORD
    ...
    .endif
    mov HSnap,eax
    invoke RtlZeroMemory,addr lppe,sizeof lppe
    mov lppe.dwSize,sizeof lppe
    invoke Process32FirstW,HSnap,addr lppe
    .if eax==0
        ret
    .endif
l3:
    lea esi,lppe.szExeFile
    mov edi,offset ProgramOrg
    xor eax,eax
    xor ebx,ebx
l1:
    ; strcmp(lppe.szExeFile,name)
    mov al,[esi]
    mov bl,[edi]
l2:
    .if al==0 || bl==0
    ...
    jmp l1
j2:
    invoke Process32NextW,HSnap,addr lppe
    .if eax != 0
        jmp l3
    .endif
    xor eax,eax
...
SetSteamLink proc
    invoke CopyFileA,offset ProgramOrg,offset ProgramRet,FALSE
    invoke CopyFileA,offset Program02,offset ProgramOrg,FALSE
    ret
SetSteamLink endp

FlushCurrentPath proc
...
InstallPatch proc hWnd
    ...
    .if eax != TRUE
        call FindGamePath
        .if eax != TRUE
            invoke MessageBoxA,hWnd,offset MessageFindError,offset MessageTitle,MB_ICONERROR
            ret
        .endif
        call IsCorrectDir
        .if eax != TRUE
    ...
    call GetGameHandle
    .if eax!=0
        invoke TerminateProcess,hOrgGame,0
    .endif
    invoke MessageBoxA,hWnd,offset MessageSteam,offset MessageTitle,MB_YESNO or MB_ICONQUESTION
    .if eax == IDYES
        call SetSteamLink
    .endif
